...
from usecases import uses_cases_client
...
def insert():
    ...
    try:
        if uses_cases_client.insert(Client(**json)):
...
def select():
    try:
        data=uses_cases_client.select_all()
        if data:
...
def delete():
    ...
    try:
        if uses_cases_client.delete_by_id(id):
...
def update():
    ...
    if not data:
        ...
        return {"message":"empty body"}, 400
    try:
        if uses_cases_client.update(Client(**data)):
